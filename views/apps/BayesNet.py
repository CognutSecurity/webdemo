...
class BayesNet(object):
   ...
   class SocketHandler(WebSocket):
      ...
      def received_message(self, message):
         ...
         if msg['cmd'] == 'train':
            error_msg = dict()
            Dataset._meta.database = SqliteDatabase(cherrypy.request.config['sql_db'])
      ...
      def opened(self):
         cherrypy.log(__file__.split('/')[-1] + ':' + self.__class__.__name__ + ': WebSocket %{:s} opened!'.format(str(self.sock)))
         self.socket_set.add(self)

      def closed(self, code, reason=None):
         cherrypy.log(__file__.split('/')[-1] + ':' + self.__class__.__name__ + ': WebSocket %{:s} closed!'.format(str(self.sock)))
